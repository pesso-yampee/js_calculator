...
for (let i = 0; i < numbersChildren.length; i++) {
  ...
  number.addEventListener('click', function() {
    ...
    if (result.textContent === '0') {
      result.textContent = Number(clickedNumber).toLocaleString();
    } else {
      // toLocaleString()関数を用いて数値表現を使う
      // ただ5桁以上になるとNaNになる
      result.textContent = Number(result.textContent + clickedNumber).toLocaleString();
      console.log(result);
    }
  });
}